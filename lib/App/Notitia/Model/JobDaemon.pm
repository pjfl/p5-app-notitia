...
sub status : Role(administrator) {
   ...
   my $run_state  = locm $req, $is_running ? 'Yes' : 'No';
   my $daemon_pid = $jobdaemon->daemon_pid;
   my $start_time = time2str '%Y-%m-%d %H:%M:%S', $jobdaemon->socket_ctime;
   my $version    = $jobdaemon->VERSION;

   $is_running or $daemon_pid = $start_time = 'N/A';

   p_textfield $form, 'is_running', $run_state,  { disabled => TRUE };
   p_textfield $form, 'daemon_pid', $daemon_pid, { disabled => TRUE };
   p_textfield $form, 'start_time', $start_time, { disabled => TRUE };
   p_textfield $form, 'version',    $version,    { disabled => TRUE };
